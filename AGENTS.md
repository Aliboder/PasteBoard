...
4. **完成标准**：自己先验证（编译/类型检查/测试），然后**启动 dev 测试窗口供用户验收**——每次修改后验收时，都必须为用户打开测试窗口（`npm run tauri dev`，后台启动并重定向日志到 `%TEMP%\opencode\tauri-dev*.log`），不能只口头汇报；用户亲自在窗口中验证通过才算完成。
5. **Rust 代码必须符合 rustfmt**：生成的 Rust 代码排版必须遵循 rustfmt 规范（不自定义缩进/换行/空格）；**任何 Rust 改动提交前必须运行 `cargo fmt`**（CI 会执行 `cargo fmt --check`，格式不通过直接阻断流水线）。曾因手写排版不合规范导致 CI 失败。
6. **git 操作主动代做**：用户不会用 git，涉及提交/推送/发布要主动协助。常规节奏：每批改动用户验收后，提交并推送；版本发布时同步打 tag + GitHub Release。
